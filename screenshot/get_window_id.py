...
try:
	...
	from Quartz import kCGWindowListOptionOnScreenAboveWindow
	from Quartz import kCGWindowListOptionOnScreenBelowWindow
...
WindowInfo = dict[str, str | int]

WINDOW_OPTIONS = {
	"all_windows": kCGWindowListOptionAll,
...
def build_option_bitmask(*opts: str) -> int:
...
def gen_window_ids(
	parent: str, title: str = "", options: str = USER_OPTS_STR, relative_to: int = kCGNullWindowID
) -> Iterator[int]:
